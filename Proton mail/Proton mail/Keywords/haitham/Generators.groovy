...
public class Generators {
	...
	def generateEmail(def emailDomain='automation',def emailLength=9) {
		try{
			...
			println('An error occured on generateEmail method from Generators')
	...
	def generateStrongPassword(def passwordLength=12,def startWithCapital=true,def includeSpecialChar=true,def includeNumbers=true) {
		try {
			...
			println('An error occured on generateStrongPassword method from Generators')
			return null
		}
	}
	@Keyword
	def generateComplexRandomText(def textLength=9, def includeSpecialChar=false, def includeNumbers=true) {
		try {
			def text='',alpha='A'..'Z',minValue=0, maxValue=alpha.size()
			if(includeNumbers==true)
				text+=new Random().with {(1..textLength).collect {(('a'..'z')+('A'..'Z')+(0..9)).join()[ nextInt((('a'..'z')+('A'..'Z')+(0..9)).join().length())]}.join()}
			else if (includeNumbers==false)
				text+=new Random().with {(1..textLength).collect {(('a'..'z')).join()[ nextInt((('a'..'z')).join().length())]}.join()}
			if(includeSpecialChar==true)
				text+='$'
			return text
		}
		catch (Exception e) {
			println('An error occured on generateComlexRandomText method from Generators')
			return null
		}
	}
}
